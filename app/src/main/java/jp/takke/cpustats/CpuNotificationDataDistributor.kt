...
internal object CpuNotificationDataDistributor {
  ...
  fun distributeNotificationData(
    cpuUsages: IntArray,
    coreDistributionMode: Int
  ): Array<CpuNotificationData> {

    // cpuUsages の index=0 は「全CPU使用率の平均」
    val coreCount = cpuUsages.size - 1

    return when (coreDistributionMode) {
      C.CORE_DISTRIBUTION_MODE_1ICON_UNSORTED -> distributeNotificationData_1IconUnsorted(
        coreCount,
        cpuUsages
      )

      C.CORE_DISTRIBUTION_MODE_1ICON_SORTED -> distributeNotificationData_1IconSorted(
        coreCount,
        cpuUsages
      )
  ...
  private fun distributeNotificationData_1IconUnsorted(
    coreCount: Int,
    cpuUsages: IntArray
  ): Array<CpuNotificationData> {
  ...
  private fun distributeNotificationData_1IconSorted(
    coreCount: Int,
    cpuUsagesIn: IntArray
  ): Array<CpuNotificationData> {
  ...
  private fun distributeNotificationData_2Icons(
    coreCount: Int,
    cpuUsages: IntArray
  ): Array<CpuNotificationData> {
