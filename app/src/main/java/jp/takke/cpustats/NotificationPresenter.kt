...
internal class NotificationPresenter(service: Service, private val mConfig: MyConfig) {
  ...
  fun updateNotifications(
    ...
    if (cpuUsages != null && mConfig.showUsageNotification) {
      ...
      val data = CpuNotificationDataDistributor.distributeNotificationData(
        cpuUsages,
        mConfig.coreDistributionMode
      )
      ...
      if (data.isNotEmpty()) {
        doNotify(
          MY_USAGE_NOTIFICATION_ID1,
          makeUsageNotification(data[0], pendingIntent).build(),
          service,
          nm,
          requestForeground
        )
      ...
      if (data.size >= 2) {
        doNotify(
          MY_USAGE_NOTIFICATION_ID2,
          makeUsageNotification(data[1], pendingIntent).build(),
          service,
          nm,
          requestForeground
        )
    ...
    if (currentCpuClock > 0 && mConfig.showFrequencyNotification) {
      ...
      doNotify(
        MY_FREQ_NOTIFICATION_ID,
        makeFrequencyNotification(minFreq, maxFreq, currentCpuClock, pendingIntent).build(),
        service,
        nm,
        requestForeground
      )
...
//        }
  ...
  private fun makeUsageNotification(
    data: CpuNotificationData,
...
//            sb.append("Cores: ");
  ...
  private fun makeFrequencyNotification(
    minFreq: Int, maxFreq: Int,
    currentCpuClock: Int, pendingIntent: PendingIntent
  ): NotificationCompat.Builder {
    ...
    val notificationContent =
      "Max Freq " + MyUtil.formatFreq(maxFreq) + " Min Freq " + MyUtil.formatFreq(minFreq)
